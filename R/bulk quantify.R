...
frame_max = max(summary.df$frame_id)

pdf("raw_plots.pdf", onefile = TRUE, width = 12, height = 9)
...
for (i in 1:nrow(roi_list))
{
  ...
  fulldata_mean_mini = fulldata_mean_mini %>% group_by(vessel, frame_id) %>% mutate(p_mean = mean(p_mean)) %>% ungroup()
  ...
  theplot = ggplot(fulldata_mean_mini) +
    ...
    scale_x_continuous(limits = c(0,frame_max))
...
for (i in 1:nrow(roi_list))
{
  ...
  fulldata_mean_mini = summary.df %>% filter(site == roi$site, animal == roi$animal, treatment == roi$treatment)
  fulldata_mean_mini = fulldata_mean_mini %>% group_by(vessel, frame_id, source_video) %>% summarise(p_mean = mean(p_mean)) %>% ungroup()
